...
class JWT
{
	 ...
	 */
	public static $leeway = 5;  // 5 seconds leeway for clock skew
	...
	/**
	 * Parse a JWT string into a PHP object without signature verification
	 *
	 ...
	 */
	public static function parse($jwt,array $allowed_algs = array())
	{
		...
		if (empty($payload->iss)){
			throw new Exception('Missing iss (anvack) in payload');
		}
		if (empty($payload->vid)){
			throw new Exception('Missing video_id in payload');
		}
		return $payload;
	}
	...
	public static function create_tkx_token($video_id, $anvack_public, $anvack_private, $user_pars = null, $ad_pars = null) {
		$now = time();
		$token = array(
			"iss" => $anvack_public,
			"vid" => $video_id,
		...
		if (is_array($user_pars)) {
			$token['user_pars'] = $user_pars;
		}
		if (is_array($ad_pars)) {
			$token['ad_pars'] = $ad_pars;
		}

		// Will throw exception if anything is wrong
		return self::encode($token,$anvack_private, 'HS256');
	}
}

// TODO::: remove
function decode_JWT($jwt,$anvack_secret){
	$err = "";
	$payload = "";
	try {
		$payload = JWT::decode($jwt, $anvack_secret, array('HS256'));
	} catch (Exception $e) {
		$err= $e->getMessage();
	}
	return array($payload,$err);
}
...
function parse_JWT($jwt){
	...
	$payload = "";
	try {
		$payload = JWT::parse($jwt,  array('HS256'));
	} catch (Exception $e) {
		$err = $e->getMessage();
	}
	return array($payload,$err);
